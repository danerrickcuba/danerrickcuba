...
if [ -e $KERNELDIR/arch/arm/boot/Image ]; then
	rm $KERNELDIR/arch/arm/boot/Image;
fi;
if [ -e $KERNELDIR/arch/arm/boot/dt.img ]; then
	rm $KERNELDIR/arch/arm/boot/dt.img;
fi;
...
rm -rf $KERNELDIR/arch/arm/boot/dts/*.dtb;
...
if [ $USER != "root" ]; then
	make CONFIG_DEBUG_SECTION_MISMATCH=y -j10 Image ARCH=arm
else
	make -j10 Image ARCH=arm
fi;

if [ -e $KERNELDIR/arch/arm/boot/Image ]; then
	echo
	echo "${bldcya}***** Final Touch for Kernel *****${txtrst}"
	stat $KERNELDIR/arch/arm/boot/Image || exit 1;
	mv ./arch/arm/boot/Image ./$BK/$TARGET
	echo
	echo "--- Creating custom dt.img ---"
	./utilities/dtbtool -o dt.img -s 2048 -p ./scripts/dtc/dtc ./arch/arm/boot/dts/
else
...
# make modules
make -j10 modules ARCH=arm  || exit 1;
